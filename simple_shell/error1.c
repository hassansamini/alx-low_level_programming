...
*/
char *strcat_cd(datashell *datash, char *msg, char *error, char *ver_str)
{
char *error_flags;
...
if (datash->args[1][0] == '-')
{
error_flags = (char *)malloc(sizeof(char) * 4);
if (!error_flags)
{
free(error);
return (NULL);
}
snprintf(error_flags, 4, "-%c", datash->args[1][1]);
strcat(error, error_flags);
free(error_flags);
}
else
...
char *error_many_args(datashell *datash)
{
char *error, *ver_str;
int size;
ver_str = aux_itoa(datash->counter);
